...
def filter_image(img_):
    ...
    # perform blurring
    #bin_img = cv.GaussianBlur(bin_img, ksize=(3,3), sigmaX=9)
    #mask = bin_img > 0
    #bin_img = np.zeros((128,128))
    #bin_img[mask] = 255

    # perform convex fill
    #idx = np.where(bin_img > 0)
    #if len(idx[0]) > 0:
    #    xmin, xmax = np.min(idx[0]), np.max(idx[0])
    #    ymin, ymax = np.min(idx[1]), np.max(idx[1])
    #    bin_img[xmin:xmax, ymin:ymax] = 255

    # perform open/close operations
    #N = 3
    #bin_img = cv.dilate(bin_img, None, iterations=N)
